...
```
Para testar a associação de interesse entre as variáveis é estabelecida as seguintes hipóteses: H0: ausência de associação e H1: presença de associação

O teste de hipótese entre a variável **Situação** e a variável **Mobilidade**, temos que a estatística de teste é igual a `r as.numeric(a$Est)`, com p-valor igual a  `r as.numeric(a$p)` rejeitamos H0 ao nível de significância de $1\%$, logo há evidências de que há associação entre as variáveis, o que também pode ser observado no intervalo de confiança de prevalências `r as.numeric(a$ICPrevalencia)`, onde 1 não está contido no intervalo, reforçando esta associação.

O teste de hipótese entre a variável **Situação** e a variável **Idade**, tem-se que a estatística de teste é igual a `r as.numeric(b$Est)` com o p valor igual a `r as.numeric(b$p)` a correlação é igual a `r as.numeric(b$cor)`, é possível concluir que conforme a classe de idades diminui, ou seja, os alunos mais novos, aumenta se a *chance* do aluno evadir.  n sei se posso dizer chance, ou nivel 

Para o teste de hipótese entre a variável **Situação** e a variável **Turno**, temos que a estatística de teste é igual a `r as.numeric(c$Est)`, com p-valor igual a `r as.numeric(c$p)`, ou seja, rejeita-se H0 com todos os níveis de significância, assim há evidências de que há associação entre as variáveis.

O teste de hipótese entre a variável **Situação** e a variável **Sexo**, temos que 
a estatística de teste é igual a `r as.numeric(d$Est)`, com p-valor igual a  `r as.numeric(c$p)`, sendo assim, rejeitamos H0 a todos os níveis de significância, logo há evidências de que há associação entre as variáveis, o que também pode ser observado no intervalo de confiança de prevalências `r as.numeric(a$ICPrevalencia)`, onde 1 não está contido no intervalo, reforçando esta associação.

O teste de hipótese entre a variável **Situação** e a variável **Nacionalidade**, temos que a estatística de teste é igual a `r as.numeric(e$Est)`, com p-valor igual a  `r as.numeric(e$p)` rejeitamos H0 a todos os níveis de significância usuais, logo há evidências de associação entre as variáveis.
